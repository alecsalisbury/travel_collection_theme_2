...
var Responsive = class {
  static matchesBreakpoint(breakpoint) {
    switch (breakpoint) {
      ...
      case "large-tablet":
        return window.matchMedia("screen and (max-width: 979px)").matches;
      case "pocket":
...
var SearchBar = class {
   ...
   */
  _onFocusOut(event2) {
    if (Responsive.matchesBreakpoint("large-tablet")) {
      return;
    }
    if (Responsive.matchesBreakpoint("large-tablet") || event2.target.classList.contains("search-bar__input") || event2.target.closest(".search-bar__inner")) {
      return;
    }
    setTimeout(() => {
      this.element.classList.remove("is-fixed");
...
var BlogSearchBar = class {
  constructor(element) {
    // Handle null element like SearchBar does
    if (!element) {
      return;
    }
    ...
    this.element = element;
    this.delegateElement = new main_default(this.element);
    this.searchBarElement = this.element.querySelector(".blog-search-bar");
    this.inputElement = this.element.querySelector('.blog-search-bar__input');
    this.searchResultsElement = this.element.querySelector(".blog-search-bar__results");
    
    this._attachListeners();
  }
  
  destroy() {
    if (this.delegateElement) {
      this.delegateElement.off();
    }
  }
  
  _attachListeners() {
    // Use event delegation exactly like SearchBar does
    this.delegateElement.on("click", '[data-action="clear-blog-input"]', this._clearInput.bind(this));
    this.delegateElement.on("input", '.blog-search-bar__input', this._debounce(this._doSearch.bind(this), 250));
    this.delegateElement.on("submit", '.blog-search-bar', this._onFormSubmit.bind(this));
    document.addEventListener("click", this._onFocusOut.bind(this));
  }
  
  _clearInput() {
    console.log('Blog search: Clearing input');
    ...
    if (this.searchBarElement) {
      this.searchBarElement.classList.remove("is-expanded", "is-loading");
    }
  }
  
  _onFocusOut(event) {
    console.log('Blog search: Focus out event', event.target);
    
    // Check if the click is inside the blog search bar
    if (event.target.closest(".blog-search-bar")) {
      return;
    }
    
    // Hide results and remove expanded state when clicking outside
    setTimeout(() => {
      if (this.searchResultsElement) {
        this.searchResultsElement.setAttribute("aria-hidden", "true");
      }
  ...
  _doSearch() {
    if (!this.inputElement || !this.searchResultsElement || !this.searchBarElement) {
      return;
    }
    ...
    let queryOptions = { method: "GET", credentials: "same-origin" };
    let url = `${window.Shopify.routes.root}search${this._supportsPredictiveApi() ? "/suggest" : ""}`;
    let articleQuery = encodeURIComponent(this.lastInputValue);
    
    // Use JSON API to get article data with author information
    let jsonUrl = `${url}.json?q=${articleQuery}&resources[type]=article&resources[limit]=10&resources[limit_scope]=each&resources[options][fields]=title,body,author`;
    console.log('Blog search JSON URL:', jsonUrl);
    
    fetch(jsonUrl, queryOptions).then((response) => {
      console.log('Blog search JSON response:', response);
      if (this.lastInputValue !== currentInput) {
        return;
      }
      return response.json();
    }).then((data) => {
      console.log('Blog search JSON data:', data);
      this.searchBarElement.classList.remove("is-loading");
      
      // Process the JSON response to create HTML
      let filteredContent = this._createBlogResultsFromJson(data);
      console.log('Filtered content from JSON:', filteredContent);
      this.searchBarElement.querySelector(".blog-search-bar__results-inner").innerHTML = filteredContent;
    }).catch((error) => {
      console.error('Blog search JSON error:', error);
      this.searchBarElement.classList.remove("is-loading");
      this.searchResultsElement.setAttribute("aria-hidden", "true");
    });
  }
  ...
  _supportsPredictiveApi() {
    let featuresScript = document.getElementById("shopify-features");
    if (featuresScript) {
      return JSON.parse(featuresScript.innerHTML)["predictiveSearch"];
    }
    return false;
  }
  
  _createBlogResultsFromJson(data) {
    console.log('Creating blog results from JSON data:', data);
    
    if (!data || !data.resources || !data.resources.results || !data.resources.results.articles) {
      return '<div class="blog-search-bar__empty-state"><p class="heading h4">No articles found</p></div>';
    }
    ...
    // Create product-style container
    blogResults += '<div class="blog-search-bar__result-products">';
    
    articles.forEach(article => {
      let title = article.title || 'Untitled Article';
      let url = article.url || '#';
      let author = article.author || 'Unknown Author';
      let imageUrl = article.image || 'https://via.placeholder.com/120x120/f0f0f0/666?text=Article';
      ...
      // Create product-style layout for each article
      blogResults += `
        <a href="${url}" class="blog-search-bar__result-item">
          <div class="blog-search-bar__image-container">
            <div class="aspect-ratio aspect-ratio--square">
              <img src="${imageUrl}" alt="${title}" width="120" height="120">
            </div>
          </div>
          <div class="blog-search-bar__item-info">
            <p class="blog-search-bar__item-title">${title}</p>
            <div class="blog-search-bar__item-meta">
              <span class="blog-search-bar__item-date">${publishedDate}</span> • <span class="blog-search-bar__item-author">${author}</span>
            </div>
          </div>
        </a>
      `;
    ...
    blogResults += '</div>'; // Close result-products container
    
    // Add "View all results" button
    blogResults += `<button class="blog-search-bar__view-all" type="submit">View all results <svg class="icon icon--arrow-right" viewBox="0 0 8 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2.90906 6L1.50906 4.6L4.10906 2L1.50906 -0.6L2.90906 -2L6.90906 2L2.90906 6Z" fill="currentColor"/></svg></button>`;
    
    console.log('Generated blog results from JSON:', blogResults);
    return blogResults;
  }

  _filterForBlogResults(searchContent) {
    console.log('Filtering search content for blog results:', searchContent);
    
    let blogResults = '';
    let foundArticles = false;
    
    // Find all sections in the search content
    let allCategories = searchContent.querySelectorAll('.search-bar__result-category');
    
    // Look specifically for blog posts/articles section
    allCategories.forEach((category, index) => {
      let categoryText = category.textContent.trim().toLowerCase();
      
      // Check if this is the blog posts/articles category
      if (categoryText.includes('blog') || categoryText.includes('article')) {
        foundArticles = true;
        
        // Get the next element which should be the list of articles
        let nextElement = category.nextElementSibling;
        if (nextElement && nextElement.classList.contains('search-bar__result-linklist')) {
          // Create product-style container
          blogResults += '<div class="blog-search-bar__result-products">';
          
          let articleLinks = nextElement.querySelectorAll('a');
          articleLinks.forEach(link => {
            ...
            blogResults += `
              <a href="${url}" class="blog-search-bar__result-item">
                <div class="blog-search-bar__image-container">
                  <div class="aspect-ratio aspect-ratio--square">
                    <img src="https://via.placeholder.com/120x120/f0f0f0/666?text=Article" alt="Article image" width="120" height="120">
                  </div>
                </div>
                <div class="blog-search-bar__item-info">
                  <p class="blog-search-bar__item-title">${title}</p>
                  <div class="blog-search-bar__item-meta">
                    <span class="blog-search-bar__item-date">Recent</span> • <span class="blog-search-bar__item-author">Author</span>
                  </div>
                </div>
              </a>
            `;
    ...
    // If no articles found, check if there are any results at all
    if (!foundArticles) {
      // Look for any linklist that might contain articles
      let allLinklists = searchContent.querySelectorAll('.search-bar__result-linklist');
      if (allLinklists.length > 0) {
        // Take the first linklist and assume it might be articles
        let firstLinklist = allLinklists[0];
        let links = firstLinklist.querySelectorAll('a');
    ...
    // Look for "View all results" button and convert it
    let viewAllButton = searchContent.querySelector('.search-bar__view-all');
    if (viewAllButton && foundArticles) {
      blogResults += `<button class="blog-search-bar__view-all" type="submit">${viewAllButton.innerHTML}</button>`;
    }
    
    // If no results, show empty state
    if (!foundArticles) {
      blogResults = '<div class="blog-search-bar__empty-state"><p class="heading h4">No articles found</p></div>';
    }
    
    console.log('Generated blog results:', blogResults);
    return blogResults;
  }
  
  _onFormSubmit(event) {
    // Let the form submit naturally with type=article
  }
...
// js/sections/FooterLinks.js
var FooterLinksSection = class {
  constructor(element) {
    this.element = element;
    if (Shopify.designMode) {
      this.element.classList.remove("hidden-lap-and-up");
...
// js/sections/TabArticlesSection.js
var import_flickity_tab_articles = __toESM(require_js());
var TabArticlesSection = class {
  constructor(element) {
    this.element = element;
    this.tabButtons = this.element.querySelectorAll('.tab-articles__tab-button');
    this.tabPanels = this.element.querySelectorAll('.tab-articles__panel');
    this.flickityInstances = new Map();
    
    this._bindEvents();
    
    // Initialize slider for the first active panel
    setTimeout(() => {
      const activePanel = this.element.querySelector('.tab-articles__panel.is-active');
      if (activePanel) {
        this._initializeSlider(activePanel);
      }
    }, 100);
  }
  ...
  _bindEvents() {
    this.tabButtons.forEach(button => {
      button.addEventListener('click', (event) => {
        ...
        const targetPanel = this.element.querySelector(`[data-tab-panel="${targetId}"]`);
        if (targetPanel) {
          targetPanel.classList.add('is-active');
          
          // Initialize slider for the newly active panel
          setTimeout(() => {
            this._initializeSlider(targetPanel);
          }, 100);
        }
      });
    });
  ...
  _initializeSlider(panel) {
    if (!panel) return;
    
    const articleList = panel.querySelector('.article-list--slider');
    if (!articleList) return;

    // Check if already initialized
    if (this.flickityInstances.has(articleList)) return;

    // Initialize Flickity with watchCSS to auto-enable based on CSS
    const flickityInstance = new import_flickity_tab_articles.default(articleList, {
      watchCSS: true,
      cellAlign: 'left',
      contain: true,
      groupCells: 3, // Group by 3 items per slide on desktop
      prevNextButtons: true,
      pageDots: false,
      draggable: !window.matchMedia("(-moz-touch-enabled: 0), (hover: hover)").matches,
      freeScroll: false,
    ...
    this.flickityInstances.set(articleList, flickityInstance);
  }
  ...
  onUnload() {
    ...
    this.flickityInstances.clear();
  }
};

// js/sections/ArticlesCollectionSection.js
var import_flickity_articles_collection = __toESM(require_js());
var ArticlesCollectionSection = class {
  constructor(element) {
    this.element = element;
  ...
  _initializeSlider() {
    const articleList = this.element.querySelector('.article-list--slider');
    if (!articleList) return;

    // Check if already initialized
    if (this.flickityInstance) return;

    // Initialize Flickity with watchCSS to auto-enable based on CSS
    this.flickityInstance = new import_flickity_articles_collection.default(articleList, {
      watchCSS: true,
      cellAlign: 'left',
      contain: true,
      groupCells: 3, // Group by 3 items per slide on desktop
...
var BlogHeroSection = class {
  constructor(element) {
    this.element = element;
    
    // Create BlogSearchBar exactly like HeaderSection creates SearchBar
    this.blogSearchBar = new BlogSearchBar(this.element.querySelector(".blog-hero__search-wrapper"));
  }
  
  onUnload() {
    if (this.blogSearchBar) {
      this.blogSearchBar.destroy();
    }
  }
};
...
(function () {
  const CANDIDATES_SELECTOR = [
    '#shopify-section-announcement-bar',
    '.announcement-bar', '.announcement',
    'header.header', '.shopify-section-header-sticky', '.header-wrapper', '.site-header', '.section-header',
    '.cookie-bar', '#CookieConsent', '.gdpr-banner', '.gdpr-modal'
  ].join(',');
  ...
  function isPinnedToTop(el) {
    ...
    const topCss = parseFloat(cs.top) || 0;
    const rect = el.getBoundingClientRect();
    return rect.top <= topCss + 1; 
  }

  function getStickyOffset() {
    let total = 0;
    document.querySelectorAll(CANDIDATES_SELECTOR).forEach(el => {
      if (!isVisible(el)) return;
      if (!isPinnedToTop(el)) return;
      total += Math.ceil(el.getBoundingClientRect().height);
    });
    return total;
  }

  function updateSafeTopVar() {
  ...
  const throttle = (fn, wait = 100) => {
    ...
    return (...args) => {
      ...
      if (now - t >= wait) {
        ...
        clearTimeout(id);
        id = setTimeout(() => { t = Date.now(); fn(...args); }, wait - (now - t));
      }
    };
  };

  document.addEventListener('DOMContentLoaded', updateSafeTopVar);
  window.addEventListener('load', updateSafeTopVar);
  window.addEventListener('resize', throttle(updateSafeTopVar, 100));
  window.addEventListener('scroll', throttle(updateSafeTopVar, 100));

  const mo = new MutationObserver(throttle(updateSafeTopVar, 50));
...
(function () {
  const ANCHOR = '#product-reviews';
  const USE_CAPTURE = true; 

  
  function resolveUrlFromWidget(el) {
    const explicit = el.getAttribute('data-redirect-url');
    ...
    const a =
      card.querySelector('a.product-item__title') ||
      card.querySelector('a.product-item__image-wrapper') ||
  ...
  function handleClick(e) {
    ...
    const widget = e.target.closest('.yotpo-widget-instance');
    if (!widget) return;

    const url = resolveUrlFromWidget(widget);
    if (!url) return;
  ...
  function handleKey(e) {
    ...
    const widget = document.activeElement?.closest?.('.yotpo-widget-instance');
    if (!widget) return;
  ...
  document.addEventListener('auxclick', handleClick, USE_CAPTURE); // middle-click
  document.addEventListener('keydown', handleKey, USE_CAPTURE);
  ...
  function enhance() {
    document.querySelectorAll('.yotpo-widget-instance').forEach(el => {
      ...
      if (!el.hasAttribute('aria-label')) el.setAttribute('aria-label', 'Ver avaliações');
    });
  }
...
(function () {
  const GOOD_HASHES = ['#product-reviews', '#yotpo-reviews-main-widget', '#reviews'];

  function findTarget() {
    return document.getElementById('product-reviews') ||
           document.getElementById('yotpo-reviews-main-widget') ||
           document.querySelector('.yotpo-reviews-main-widget') ||
           document.querySelector('#yotpo-reviews-container');
  }

  function normalizeHash() {
    if (location.hash.toLowerCase() === '#yotpo-reviews-main-widget') {
      history.replaceState(null, '', '#product-reviews');
    }
  }

  function tryScrollWithRetry() {
    if (!GOOD_HASHES.includes(location.hash.toLowerCase())) return;
    ...
    (function loop() {
      ...
      if (el) {
        ...
        setTimeout(() => {
          ...
          const adjust = el.getBoundingClientRect().top - safeTop;
          if (Math.abs(adjust) > 2) window.scrollBy({ top: adjust, behavior: 'smooth' });
        }, 350);
        return;
      }
      if (Date.now() - start < maxMs) setTimeout(loop, 200);
    })();
  }

  document.addEventListener('DOMContentLoaded', tryScrollWithRetry);
  window.addEventListener('load', tryScrollWithRetry);
  window.addEventListener('hashchange', tryScrollWithRetry);
})();

/**
 * Redirect whole .product-item to PDP on PLP/Search
 * Requires each product-item to have data-pdp-url="{{ variant_url | default: product_url }}"
 */
(function () {
  if (window.__plpPdpRedirectBound) return;
  window.__plpPdpRedirectBound = true;
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      var card = e.target.closest('.product-item[data-pdp-url]');
      if (!card) return;

      
      if (
        e.target.closest(
          'button, .button, input, select, textarea, .product-item__action-list, [data-action], [data-secondary-action]'
        )
      ) {
        return;
      }

      var pdpUrl = card.getAttribute('data-pdp-url');
      if (!pdpUrl) return;
